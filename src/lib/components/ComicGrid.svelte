...
{#if error}
	<div class="text-center py-12">
		<div class="bg-red-900/50 border border-red-700 rounded-lg p-6 max-w-md mx-auto">
			<p class="text-red-300 font-semibold mb-2">Error loading comics</p>
			<p class="text-red-400 text-sm">{error}</p>
			{#if error.includes('API key')}
				<p class="text-red-300 text-xs mt-2">
					Please configure the COMIC_VINE_API_KEY environment variable.
					<br />
					Get your free API key at <a href="https://comicvine.gamespot.com/api/" target="_blank" class="underline">comicvine.gamespot.com/api/</a>
				</p>
			{/if}
		</div>
	</div>
{:else if loading}
	<div class="flex justify-center items-center py-20">
		<div class="text-center">
			...
			<p class="text-gray-400">Searching for comics...</p>
		</div>
	</div>
{:else if results.length === 0 && hasSearched && !loading}
	<div class="text-center py-12">
		<p class="text-gray-400 text-lg">No comics found for "<span class="text-white font-semibold">{query}</span>"</p>
...
{:else if results.length > 0}
	<div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 justify-items-center">
		{#each results as comic (comic.id)}
			<div class="w-full max-w-[300px]">
				<ComicCard {comic} searchQuery={query} />
			</div>
		{/each}
	</div>
{:else}
