		...
		<div class="flex items-center space-x-2 relative">
			{#if download.link && download.progress >= 100}
				...
				<button
					...
					disabled={renewing}
					class="inline-flex items-center px-2 py-1 rounded-md text-xs font-medium border border-amber-600/80 text-amber-200 hover:bg-amber-900/40 transition-colors disabled:opacity-50"
					title="Get a fresh direct URL from Real-Debrid (links expire, or fix a stuck real-debrid.com page)"
				>
					{renewing ? '…' : 'Renew'}
				</button>
				<button
