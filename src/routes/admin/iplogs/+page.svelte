...
<div class="min-h-screen py-8 px-4" style="background: linear-gradient(to bottom right, var(--theme-bg-primary), var(--theme-bg-secondary), var(--theme-bg-primary));">
	<div class="mx-auto max-w-7xl">
		...
		<div class="mb-8">
			<div class="flex items-center justify-between mb-4">
				<div>
					...
					<p class="text-gray-400">Logged in as <span class="text-blue-400 font-semibold">{username}</span></p>
				</div>
			...
			<div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
				<div class="bg-gradient-to-br from-blue-600 to-blue-700 rounded-lg p-6 shadow-lg">
					<div class="text-blue-100 text-sm font-medium mb-1">Total IPs Logged</div>
					<div class="text-white text-3xl font-bold">{totalIps.toLocaleString()}</div>
				</div>
				<div class="bg-gradient-to-br from-green-600 to-green-700 rounded-lg p-6 shadow-lg">
					<div class="text-green-100 text-sm font-medium mb-1">Unique Countries</div>
					<div class="text-white text-3xl font-bold">{uniqueCountries}</div>
				</div>
				<div class="bg-gradient-to-br from-purple-600 to-purple-700 rounded-lg p-6 shadow-lg">
					<div class="text-purple-100 text-sm font-medium mb-1">Filtered Results</div>
					<div class="text-white text-3xl font-bold">{filteredCount.toLocaleString()}</div>
				</div>
			</div>

			<!-- Filters -->
			<div class="backdrop-blur-sm rounded-lg p-6 shadow-lg border" style="background-color: var(--theme-bg-secondary); border-color: var(--theme-border);">
		...
		<div class="backdrop-blur-sm rounded-lg shadow-lg border overflow-hidden" style="background-color: var(--theme-bg-secondary); border-color: var(--theme-border);">
							...
							<tr class="admin-table-row transition-colors">
								...
								<td class="px-6 py-4">
									<div class="flex items-center gap-2">
										<span class="text-2xl">{getCountryFlag(log.location)}</span>
										<span class="text-sm text-gray-300">{log.location}</span>
									</div>
								</td>
								<td class="px-6 py-4">
			...
			<!-- Pagination -->
			{#if totalPages > 1}
				<div class="border-t px-6 py-4" style="background-color: var(--theme-bg-primary); border-color: var(--theme-border);">
					<div class="flex items-center justify-between">
						...
						<div class="flex gap-2">
							<button
								...
								class="px-3 py-1 bg-gray-700 text-white rounded hover:bg-gray-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
							>
								««
							</button>
							<button
							...
							{#each Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
								...
								<button
									...
									class="px-3 py-1 rounded transition-colors {currentPage === page 
